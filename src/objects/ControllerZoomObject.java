...
public class ControllerZoomObject extends LevelObject {
	
	public int ID,pallete = 0;
	public long parts = 1;
	public double[] position = {0,0},scale = {1,1};
	public double rotation = 0,targetZoom = 1.5,zoomTime = 1;
	public boolean enabled = true,visible = true;
	
	public ControllerZoomObject(String data,ConversionType type) throws Exception {
		super(data,type);
		
		defaultValues = new Object[] {ID,pallete,position,scale,rotation,enabled,visible,targetZoom,zoomTime,parts};
	...
	void setupValues() {
		...
		targetZoom = (objectData[7] == null) ? targetZoom:(double) Double.valueOf(String.valueOf(objectData[7].getValue()));
		zoomTime = (objectData[8] == null) ? zoomTime:(double) Double.valueOf(String.valueOf(objectData[8].getValue()));
		parts = (objectData[9] == null) ? parts:(long) objectData[9].getValue();
		...
		if (objectData[7] == null) { objectData[7] = new FloatType(Utility.floatToString(targetZoom));}
		if (objectData[8] == null) { objectData[8] = new FloatType(Utility.floatToString(zoomTime));}
		if (objectData[9] == null) { objectData[9] = new IntegerType(Utility.integerToString(parts));}
