...
public class CoinYellowObject extends LevelObject {
	
	public int ID,pallete = 0;
	public double[] position = {0,0},scale = {1,1},velocity = {0,0};
	public double rotation = 0;
	public boolean enabled = true,visible = true,physics = false;
	
	public CoinYellowObject(String data,ConversionType type) throws Exception {
		super(data,type);
		
		defaultValues = new Object[] {ID,pallete,position,scale,rotation,enabled,visible,physics,velocity};
	...
	void setupValues() {
		...
		physics = (objectData[7] == null) ? physics:(boolean) objectData[7].getValue();
		velocity = (objectData[8] == null) ? velocity:(double[]) objectData[8].getValue();
		...
		if (objectData[7] == null) { objectData[7] = new BooleanType(Utility.booleanToString(physics));}
		if (objectData[8] == null) { objectData[8] = new Vector2Type(Utility.vector2ToString(velocity,false));}
	}
