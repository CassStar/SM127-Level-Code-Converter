...
public class NPCYoshiObject extends LevelObject {
	
	public int ID,pallete = 0;
	public long idleExpression = 0,idleAction = 0,speakingExpression = 1,speakingAction = 0,requiredShines = 0;
	public double[] position = {0,0},scale = {1,1},skinColour = {0,1,0,1},shoeColour = {1,0.27,0,1};
	public double rotation = 0,walkSpeed = 0;
	public boolean enabled = true,visible = true,moveType = true,physicsEnabled = true,pathReference = false,rainbow = false;
	public String tagLink = "";
	public double[][] path = {{-50,-50},{50,-50}},pathDuplicate = {{-50,-50},{50,-50}};
	
	public NPCYoshiObject(String data,ConversionType type) throws Exception {
		super(data,type);
		...
		defaultValues = new Object[] {ID,pallete,position,scale,rotation,enabled,visible,path,pathDuplicate,moveType,walkSpeed,physicsEnabled,
				idleExpression,idleAction,speakingExpression,speakingAction,pathReference,tagLink,requiredShines,skinColour,shoeColour,
				rainbow};
	...
	void setupValues() {
		...
		objectData = Utility.ensureMinimumLength(objectData,22);
		...
		skinColour = (objectData[19] == null) ? skinColour:(double[]) objectData[19].getValue();
		shoeColour = (objectData[20] == null) ? shoeColour:(double[]) objectData[20].getValue();
		rainbow = (objectData[21] == null) ? rainbow:(boolean) objectData[21].getValue();
		...
		if (objectData[19] == null) { objectData[19] = new ColourType(Utility.colourToString(skinColour));}
		if (objectData[20] == null) { objectData[20] = new ColourType(Utility.colourToString(shoeColour));}
		if (objectData[21] == null) { objectData[21] = new BooleanType(Utility.booleanToString(rainbow));}
	}
