...
public class WaterSpongeObject extends LevelObject {
	...
	public long waterDrainRange = 64;
	public double[] position = {0,0},scale = {1,1};
	public double rotation = 0,waterDrainSpeed = 15;
	public boolean enabled = true,visible = true;
	
	public WaterSpongeObject(String data,ConversionType type) throws Exception {
		super(data,type);
		
		defaultValues = new Object[] {ID,pallete,position,scale,rotation,enabled,visible,waterDrainSpeed,waterDrainRange};
	...
	void setupValues() {
		...
		waterDrainSpeed = (objectData[7] == null) ? waterDrainSpeed:(double) Double.valueOf(String.valueOf(objectData[7].getValue()));
		waterDrainRange = (objectData[8] == null) ? waterDrainRange:(long) objectData[8].getValue();
		...
		if (objectData[7] == null) { objectData[7] = new FloatType(Utility.floatToString(waterDrainSpeed));}
		if (objectData[8] == null) { objectData[8] = new IntegerType(Utility.integerToString(waterDrainRange));}
	}
}
