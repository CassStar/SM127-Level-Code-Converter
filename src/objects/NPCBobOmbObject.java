...
public class NPCBobOmbObject extends LevelObject {
	
	public int ID,pallete = 0;
	public long idleExpression = 0,idleAction = 0,speakingExpression = 1,speakingAction = 0;
	public double[] position = {0,0},scale = {1,1},colour = {1,0,0.39,1},eyeColour = {1,1,1,1};
	...
	public NPCBobOmbObject(String data,ConversionType type) throws Exception {
		super(data,type);
		...
		defaultValues = new Object[] {ID,pallete,position,scale,rotation,enabled,visible,path,pathDuplicate,moveType,walkSpeed,physicsEnabled,
				idleExpression,idleAction,speakingExpression,speakingAction,pathReference,tagLink,colour,rainbow,eyeColour};
	...
	void setupValues() {
		...
		objectData = Utility.ensureMinimumLength(objectData,20);
		...
		colour = (objectData[18] == null) ? colour:(double[]) objectData[18].getValue();
		rainbow = (objectData[19] == null) ? rainbow:(boolean) objectData[19].getValue();
		...
		if (objectData[18] == null) { objectData[18] = new ColourType(Utility.colourToString(colour));}
		if (objectData[19] == null) { objectData[19] = new BooleanType(Utility.booleanToString(rainbow));}
		
		if (objectData.length > 20) {
			
			eyeColour = (objectData[20] == null) ? colour:(double[]) objectData[20].getValue();
		}
	}
