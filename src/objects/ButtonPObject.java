...
public class ButtonPObject extends LevelObject {
	
	public int ID,pallete = 0;
	public double[] position = {0,0},scale = {1,1};
	public double rotation = 0,pressedTime = 20;
	public boolean enabled = true,visible = true;
	
	public ButtonPObject(String data,ConversionType type) throws Exception {
		super(data,type);
		
		defaultValues = new Object[] {ID,pallete,position,scale,rotation,enabled,visible,pressedTime};
	...
	void setupValues() {
		...
		pressedTime = (objectData[7] == null) ? pressedTime:(double) Double.valueOf(String.valueOf(objectData[7].getValue()));
		...
		if (objectData[7] == null) { objectData[7] = new FloatType(Utility.floatToString(pressedTime));}
	}
