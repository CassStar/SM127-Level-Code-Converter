...
public class BlockOnOffControlledObject extends LevelObject {
	
	public int ID,pallete = 0;
	public double[] position = {0,0},scale = {1,1};
	public double rotation = 0;
	public boolean enabled = true,visible = true,inverted = false;
	
	public BlockOnOffControlledObject(String data,ConversionType type) throws Exception {
		super(data,type);
		
		defaultValues = new Object[] {ID,pallete,position,scale,rotation,enabled,visible,inverted};
	...
	void setupValues() {
		...
		if (objectData.length > 7) {
			...
			inverted = (objectData[7] == null) ? inverted:(boolean) objectData[7].getValue();
		}
	}
