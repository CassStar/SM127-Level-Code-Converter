...
public class NipperFireBallObject extends LevelObject {
	...
	public boolean enabled = true,visible = true,bouncy = false;
	
	public NipperFireBallObject(String data,ConversionType type) throws Exception {
		super(data,type);
		
		defaultValues = new Object[] {ID,pallete,position,scale,rotation,enabled,visible,velocity,bouncy};
	...
	void setupValues() {
		...
		bouncy = (objectData[8] == null) ? bouncy:(boolean) objectData[8].getValue();
		...
		if (objectData[8] == null) { objectData[8] = new BooleanType(Utility.booleanToString(bouncy));}
	}
