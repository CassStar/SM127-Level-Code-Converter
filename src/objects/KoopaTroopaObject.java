...
public class KoopaTroopaObject extends LevelObject {
	...
	public boolean enabled = true,visible = true,rainbow = false,troopaType = false,shelled = false;
	
	public KoopaTroopaObject(String data,ConversionType type) throws Exception {
		super(data,type);
		
		defaultValues = new Object[] {ID,pallete,position,scale,rotation,enabled,visible,colour,rainbow,troopaType,shelled};
	...
	void setupValues() {
		...
		colour = (objectData[7] == null) ? colour:(double[]) objectData[7].getValue();
		rainbow = (objectData[8] == null) ? rainbow:(boolean) objectData[8].getValue();
		troopaType = (objectData[9] == null) ? troopaType:(boolean) objectData[9].getValue();
		...
		if (objectData[7] == null) { objectData[7] = new ColourType(Utility.colourToString(colour));}
		if (objectData[8] == null) { objectData[8] = new BooleanType(Utility.booleanToString(rainbow));}
		if (objectData[9] == null) { objectData[9] = new BooleanType(Utility.booleanToString(troopaType));}
		
		if (objectData.length > 10) {
		
			shelled = (objectData[10] == null) ? shelled:(boolean) objectData[10].getValue();
		}
	}
