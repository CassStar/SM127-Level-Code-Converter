...
public class NPCToadObject extends LevelObject {
	
	public int ID,pallete = 0;
	public long idleExpression = 0,idleAction = 0,speakingExpression = 1,speakingAction = 0,requiredShines = 0;
	public double[] position = {0,0},scale = {1,1},spotsColour = {1,0,0,1},coatColour = {0,0,1,1};
	...
	public NPCToadObject(String data,ConversionType type) throws Exception {
		super(data,type);
		...
		defaultValues = new Object[] {ID,pallete,position,scale,rotation,enabled,visible,path,pathDuplicate,moveType,walkSpeed,physicsEnabled,
				idleExpression,idleAction,speakingExpression,speakingAction,pathReference,tagLink,requiredShines,spotsColour,coatColour,
				rainbow};
	...
	void setupValues() {
		...
		spotsColour = (objectData[19] == null) ? spotsColour:(double[]) objectData[19].getValue();
		coatColour = (objectData[20] == null) ? coatColour:(double[]) objectData[20].getValue();
		...
		if (objectData[19] == null) { objectData[19] = new ColourType(Utility.colourToString(spotsColour));}
		if (objectData[20] == null) { objectData[20] = new ColourType(Utility.colourToString(coatColour));}
