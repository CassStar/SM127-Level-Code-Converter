...
public class StarSlingObject extends LevelObject {
	
	public int ID,pallete = 0;
	public double[] position = {0,0},scale = {1,1};
	public double rotation = 0,launchPower = 10;
	public boolean enabled = true,visible = true;
	
	public StarSlingObject(String data,ConversionType type) throws Exception {
		super(data,type);
		
		defaultValues = new Object[] {ID,pallete,position,scale,rotation,enabled,visible,launchPower};
	...
	void setupValues() {
		...
		launchPower = (objectData[7] == null) ? launchPower:(double) Double.valueOf(String.valueOf(objectData[7].getValue()));
		...
		if (objectData[7] == null) { objectData[7] = new FloatType(Utility.floatToString(launchPower));}
	}
}
