...
public class WaterObject extends LevelObject {
	...
	public double[] position = {0,0},scale = {1,1},colour = {0.19,0.52,1,1};
	public double rotation = 0,width = 600,height = 300,toxicity = 0;
	...
	public WaterObject(String data,ConversionType type) throws Exception {
		...
		defaultValues = new Object[] {ID,pallete,position,scale,rotation,enabled,visible,width,height,colour,renderInFront,tag,toxicity,
				tapMode};
	...
	void setupValues() {
		...
		tag = (objectData[11] == null) ? tag:String.valueOf(objectData[11].getValue());
		toxicity = (objectData[12] == null) ? toxicity:(double) Double.valueOf(String.valueOf(objectData[12].getValue()));
		tapMode = (objectData[13] == null) ? tapMode:(boolean) objectData[13].getValue();
		...
		if (objectData[12] == null) { objectData[12] = new FloatType(Utility.floatToString(toxicity));}
		if (objectData[13] == null) { objectData[13] = new BooleanType(Utility.booleanToString(tapMode));}
	}
