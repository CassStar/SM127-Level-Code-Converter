...
public class BulletBillBlasterObject extends LevelObject {
	...
	public long chaseDirection = 0;
	public double[] position = {0,0},scale = {1,1},colour = {0,1,0,1};
	public double rotation = 0,speed = 0.75,waitTime = 3,offset = 0;
	...
	public BulletBillBlasterObject(String data,ConversionType type) throws Exception {
		...
		defaultValues = new Object[] {ID,pallete,position,scale,rotation,enabled,visible,chase,speed,colour,waitTime,invincible,chaseDirection,offset};
	...
	void setupValues() {
		...
		waitTime = (objectData[10] == null) ? waitTime:(double) Double.valueOf(String.valueOf(objectData[10].getValue()));
		invincible = (objectData[11] == null) ? invincible:(boolean) objectData[11].getValue();
		chaseDirection = (objectData[12] == null) ? chaseDirection:(long) objectData[12].getValue();
		...
		if (objectData[10] == null) { objectData[10] = new FloatType(Utility.floatToString(waitTime));}
		if (objectData[11] == null) { objectData[11] = new BooleanType(Utility.booleanToString(invincible));}
		if (objectData[12] == null) { objectData[12] = new IntegerType(Utility.integerToString(chaseDirection));}
		
		if (objectData.length > 13) {
		
			offset = (objectData[13] == null) ? offset:(double) Double.valueOf(String.valueOf(objectData[13].getValue()));
		}
	}
